...
PDF2Epub包含多种文本清理功能，用于优化转换后的内容：

1. **页码和页眉移除**：自动检测并移除页面左上角的页码和章节标题，例如"2 海德格尔《哲学献文》导论"
2. **脚注标记格式化**：将常见的脚注标记（如①②③、[1]、(1)等）转换为HTML上标格式
3. **特殊标题格式化**：识别并正确格式化"序言"、"前言"、"附录"等特殊标题
4. **段落和换行处理**：智能处理段落和换行，修复OCR过程中可能出现的问题
5. **标点符号规范化**：统一标点符号格式，确保文本一致性

这些功能特别适用于学术文献、教材和专业书籍的转换，能够有效保留原文的结构和格式。
